...
use cosmwasm_std::{
    Attribute, Binary, Decimal256, Deps, DepsMut, Empty, Env, MessageInfo, QueryRequest, Response,
    StdResult, Uint256,
};
...
pub fn run_queries(
    ...
    for (i, query) in queries.into_iter().enumerate() {
        let mut key_att = String::from("query");
        key_att.push_str(i.to_string().as_str());

        let mut key_att_request = key_att.clone();
        key_att_request.push_str("_request");

        let mut key_att_value_string = key_att.clone();
        key_att_value_string.push_str("_value");

        attributes.push(attr(key_att_request, format!("{:?}", query.request)));

        let mut value_response = perform_query(deps.as_ref(), query.request)?;
        ...
        if let Some(assert_with) = query.assert_with {
            let (_, value_string, compare_string) =
                assert_value(value_response, assert_with.clone())?;
...
pub fn perform_query(deps: Deps, request: QueryRequest<Empty>) -> StdResult<Value> {
    let res: Value = deps.querier.query(&request)?;
...
pub fn get_value_by_path(
    ...
    Ok((val.to_owned(), path_str))
}

pub fn assert_value(
    val: Value,
    assert_info: AssertInfo,
) -> Result<(bool, String, String), ContractError> {
    let val_string: String = convert_value_to_string(val)?;

    match assert_info.data_type {
        DataType::String {} => match assert_info.operator {
            AssertOperator::Equal {} => {
                if val_string == assert_info.value {
                    Ok((true, val_string, assert_info.value))
                } else {
                    Err(ContractError::AssertFailed {
                        value_origin: val_string,
                        value_to_compare: assert_info.value,
                        operator: assert_info.operator,
                    })
                }
            }
            _ => Err(ContractError::AssertTypeNotValid {
                value_type: assert_info.data_type,
                operator: assert_info.operator,
            }),
        },
        DataType::Int {} => {
...
pub fn convert_value_to_string(val: Value) -> Result<String, ContractError> {
    if val.is_string() {
        ...
        Err(ContractError::UnreconiziedType {})
    }
}
